...
הדרישה שמספר הטענות יהיה **סופי** אינה דקדקנות טכנית. בהינתן אינסוף טענות $,P_1(n),\ P_2(n),\ P_3(n),\ \dots$ שכל אחת מהן מתקיימת החל ממקום מסוים, אם המקומות $N_1,\ N_2,\ N_3,\ \dots$ שונים זה מזה אז ייתכן שהקבוצה $\left\{N_1,\ N_2,\ N_3,\ \dots\right\}$ היא אינסופית, ולקבוצה אינסופית של מספרים טבעיים אין מקסימום. במקרה כזה אין שום מקום שהחל ממנו כל הטענות מתקיימות יחד.
...
::: {#box-cor-limit-open-interval .thmrem numbered="false" title="ניסוח באמצעות קטעים פתוחים"}
ראינו זה עתה שלכל $m$ ו-$M$ המקיימים $,m < L < M$ מתקיים $m < a_n < M$ החל ממקום מסוים.

נשים לב עכשיו שהקבוצה $\left\{x \mid m < x < M\right\}$ אינה אלא הקטע הפתוח $,\left(m,\ M\right)$ והתנאי $m < L < M$ אומר בדיוק ש-$.L \in \left(m,\ M\right)$ מצד שני, גם סביבת $\varepsilon$ של $,L$ כלומר $,\left(L-\varepsilon,\ L+\varepsilon\right)$ היא קטע פתוח המכיל את $.L$ מכאן מתקבל הניסוח הבא: מתקיים $\lim\limits_{n\to\infty} a_n = L$ **אם ורק אם** לכל קטע פתוח $\left(m,\ M\right)$ המקיים $L \in \left(m,\ M\right)$ מתקיים $a_n \in \left(m,\ M\right)$ החל ממקום מסוים.
:::
...
השלימו את פרטי ההוכחה של הטענה שב[הערה על קטעים פתוחים](#box-cor-limit-open-interval): הוכיחו כי $\lim\limits_{n\to\infty} a_n = L$ אם ורק אם לכל קטע פתוח $\left(m,\ M\right)$ המקיים $L \in \left(m,\ M\right)$ קיים $N \in \mathbb{N}$ כך שלכל $n \geq N$ מתקיים $.a_n \in \left(m,\ M\right)$
:::

::: {.thmsol .foldable}
**הכיוון הראשון.** נניח $,\lim\limits_{n\to\infty} a_n = L$ ויהי $\left(m,\ M\right)$ קטע פתוח המקיים $.L \in \left(m,\ M\right)$

מהגדרת הקטע הפתוח נובע $,m < L < M$ ולכן אפשר להפעיל עליהם את [](#box-thm-limit-window).

סעיף (א) נותן $N_1 \in \mathbb{N}$ כך שלכל $n \geq N_1$ מתקיים $.a_n > m$

סעיף (ב) נותן $N_2 \in \mathbb{N}$ כך שלכל $n \geq N_2$ מתקיים $.a_n < M$

נבחר $.N = \max\left\{N_1,\ N_2\right\}$ אז לכל $n \geq N$ שני האי-שוויונות מתקיימים, כלומר $.a_n \in \left(m,\ M\right)$
