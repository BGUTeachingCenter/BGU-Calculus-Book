...
כשאנו חוקרים סדרה אינסופית, אחת השאלות הטבעיות שנולדות היא מה קורה לסדרה **בטווח הרחוק**: האם ערכי הסדרה הולכים ומתקרבים לאיזשהו ערך? השאלה הזו עולה ממספר סיבות:

- **קירוב.** לפעמים אנחנו מעוניינים ל**קרב** מספר שאיננו יודעים לרשום במדויק — למשל מספר אי-רציונלי כמו $\sqrt{2}$, שאת הצגתו העשרונית איננו יכולים לרשום במלואה. במקרה כזה אנחנו **בונים** בעצמנו סדרה שאת איבריה כן קל לחשב, והם הולכים ומתקרבים אל אותו מספר; מחשבים איברים עד שאנחנו ״מספיק קרובים״, ועוצרים.
- **תהליך שמתייצב.** ולפעמים הסדרה אינה נבנית על ידינו כלל, אלא **מתארת תהליך**: $a_n$ הוא התוצאה המתקבלת כאשר הפרמטר הוא $n$. כך, למשל, אם $a_n$ הוא הזמן הדרוש לבניית בית על ידי $n$ פועלים, אז ככל שנוסיף פועלים הזמן יילך ויקטן — אך לא לאפס, שכן יש חלק מהעבודה שאי אפשר לחלק ביניהם. הזמן מתייצב על ערך מסוים: הזמן המהיר ביותר שבו אפשר לבנות את הבית בכלל. הידיעה הזו מעשית מאוד — מרגע שאנחנו ״מספיק קרובים״ לערך הזה, כבר אין טעם להוסיף פועלים.

נתן דוגמא מפורטט

:::::: {#box-exm-pi .thmexm title="קירוב המספר $\\pi$ בעזרת מצולעים משוכללים"}
נתבונן במצולע משוכלל בעל $n$ צלעות החסום במעגל שרדיוסו $1$. שטחו הוא $$,a_n = \frac{n}{2}\,\sin\frac{2\pi}{n}$$ וככל ש-$n$ גדל, השטח שנותר בין המצולע למעגל הולך וקטן, ושטח המצולע מתקרב לשטח העיגול — כלומר ל-$\pi$.

::: {.content-visible when-format="html"}
```{=html}
<div class="pi-demo">
<div class="pi-demo-row">
<label class="pi-demo-key" for="pi-demo-slider">מספר הצלעות:</label>
<input class="pi-demo-slider" id="pi-demo-slider" type="range" min="0" max="6" value="1" step="1">
<span class="pi-demo-badge" dir="ltr">n = <span class="pi-demo-n">6</span></span>
</div>
<svg class="pi-demo-svg" viewBox="-1.15 -1.15 2.3 2.3" role="img" aria-label="מצולע משוכלל חסום במעגל היחידה">
...
for ax, n in zip(axes, ns):
    ...
    ax.fill(np.cos(a), np.sin(a), color="#c0392b", alpha=0.5, zorder=2)
    ax.plot(np.cos(a), np.sin(a), color="#c0392b", lw=1.4, zorder=4)
    ax.set_title(r"$n=%d$,   $a_n=%.4f$" % (n, (n / 2) * np.sin(2 * np.pi / n)),
                 fontsize=11)
...
תרשים: מצולעים משוכללים חסומים במעגל היחידה — ככל ש-$n$ גדל, שטח המצולע מתקרב ל-$\pi$
:::
