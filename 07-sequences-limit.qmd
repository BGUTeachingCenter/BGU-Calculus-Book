...
::: {.content-visible when-format="pdf"}
בהמשך הפרק משולבות **סימולציות** אינטראקטיביות: בוחרים בהן $\varepsilon$, מוצאים את ה-$N$ המינימלי, ורואים את הרצועה ואת האיברים החורגים. הסימולציות עצמן פועלות רק בגרסה המקוונת של הספר, בכתובת {{< var book.url >}}/07-sequences-limit.html — אך הן ממוספרות גם כאן, כך שהמספור זהה בשתי הגרסאות.
:::
...
הדרך הפורמלית הסטנדרטית לכתוב את זה היא:
...
זה נכון לכל $n\in\mathbb{N}$, ובפרט לכל $n\geq 14$. נבחר $N=14$, וקיבלנו: לכל $\varepsilon>0$ קיים $N$ *(במקרה שלנו $14$)* כך שלכל $n\geq N$ מתקיים $|a_n - 1| < \varepsilon$ — כמו שרצינו להוכיח.
...
::: {#box-q-one-over-n .thmqst title="שאיפה לאפס"}
...
::: {#box-q-pm-one .thmqst title="סדרה מתחלפת שאינה מתכנסת"}
...
::::: {#box-sim-pm-one .thmsim title="הסדרה $a_n = (-1)^n$"}
::: {.content-visible when-format="html"}
```{=html}
...
בחרו מועמד לגבול וצמצמו את $\varepsilon$: לכל מועמד נשארים אינסוף איברים מחוץ לרצועה. שימו לב שציר הערכים קבוע — הרצועה היא שגדלה, ולא הסדרה שמתקרבת.
...
כאן הגבול אינו $0$ והסדרה מתקרבת אליו מלמטה בלבד, ולכן כל האיברים החורגים נמצאים בצד אחד של הרצועה.
