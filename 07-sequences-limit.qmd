...
תהי $\{a_n\}_{n=1}^{\infty}$ סדרה. נאמר שהסדרה **מתכנסת**, או שיש לה **גבול**, אם קיים $L\in\mathbb{R}$ כך שלכל $\varepsilon>0$ קיים $N\in\mathbb{N}$ כך שלכל $n \geq N$ מתקיים $$|a_n - L| < \varepsilon$$ במקרה זה נאמר שהסדרה **שואפת** (או **מתכנסת**) ל-$L$, ונסמן $\lim\limits_{n\to\infty}a_n=L$ או $a_n \to L$. אם אין לסדרה גבול, נאמר שהיא **מתבדרת**.
:::
...
:::: {.content-visible when-format="pdf"}
::: {style="text-align:center"}
תרשים: סדרה המתכנסת ל-$L$ — האזור המוצלל הוא סביבת ה-$\varepsilon$ של $L$, כלומר הקטע $(L-\varepsilon,\,L+\varepsilon)$; כל האיברים שהאינדקס שלהם גדול או שווה ל-$N$ נמצאים בתוכה, ומחוצה לה נשארו רק האיברים המסומנים באדום
:::
::::

::: {.content-visible when-format="html"}
![סדרה המתכנסת ל-$L$ — האזור המוצלל הוא סביבת ה-$\varepsilon$ של $L$, כלומר הקטע $(L-\varepsilon,\,L+\varepsilon)$; כל האיברים שהאינדקס שלהם גדול או שווה ל-$N$ נמצאים בתוכה, ומחוצה לה נשארו רק האיברים המסומנים באדום](figures/c07_fig09.png){#fig-seq-eps-band width="80%" fig-align="center"}
:::
...
הדרך ההגיונית היחידה בשלב הזה היא לפתוח את הביטוי $|a_n - 1|$, ״לשחק איתו״, ולמצוא איזשהו אינדקס $N$ — שתלוי ב-$\varepsilon$ — שמקיים את הטענה. וזה מה שנעשה.
...
תהי $\left\{a_n\right\}_{n=1}^{\infty}$ הסדרה המוגדרת על ידי $a_n=\dfrac{1}{n}$. הראו כי $\lim\limits_{n\to\infty}a_n=0$.
:::
...
ובכן, אינטואיטיבית זה מאוד ברור — הביטוי $\frac{1}{n}$ הוא ההופכי של $n$, וכיוון ש-$n$ אינו חסום מלמעלה, $\frac{1}{n}$ הולך ומתקרב ל-$0$. ננסה לתרגם את האינטואיציה שלנו לטענה פורמלית. מכיוון ש-$n>0$ וגם $\varepsilon>0$, מותר לכפול את שני האגפים ב-$\dfrac{n}{\varepsilon}$ מבלי לשנות את כיוון האי-שוויון, ולכן

**מתקיים** $\frac{1}{n} < \varepsilon$ **אם ורק אם** $\frac{1}{\varepsilon} < n$**, ולכן די למצוא** $N\in\mathbb{N}$ **כך שלכל** $n\geq N$ **יתקיים** $\frac{1}{\varepsilon}<n$**.**
...
```{=latex}
\par\medskip
\noindent\beginL\hbox to \linewidth{\hss\includegraphics[width=0.9\linewidth]{figures/c07_fig15.png}\hss}\endL\par
\medskip
```
...
::: {.content-visible when-format="html"}
![ערך השלם. עבור $m = 3.4$ מתקיים $\lfloor m \rfloor = 3$, ולכן $3 \leq m < 4$ — מקרה פרטי של $\lfloor m \rfloor \leq m < \lfloor m \rfloor + 1$](figures/c07_fig15.png){#fig-floor width="85%" fig-align="center"}
:::
...
נשלול את התנאי שבהגדרה: קיים $\varepsilon>0$ כך שלכל $N\in\mathbb{N}$ קיימים $n,m\geq N$ שעבורם
$$.\left|a_n-a_m\right| \geq \varepsilon$$

כעת נשתמש בהנחה שהסדרה מתכנסת. לפי הגדרת הגבול, לכל $d>0$ קיים $N\in\mathbb{N}$ כך שלכל $n\geq N$ מתקיים
$$.\left|a_n-L\right| < d$$
...
עבור אותו $N$ קיימים, לפי השלילה, אינדקסים $n,m\geq N$ שעבורם $\left|a_n-a_m\right|\geq\varepsilon$. אך
$$,\varepsilon \leq \left|a_n-a_m\right| = \left|(a_n-L)+(L-a_m)\right| \leq \left|a_n-L\right|+\left|a_m-L\right| < \frac{\varepsilon}{2}+\frac{\varepsilon}{2} = \varepsilon$$
כלומר $\varepsilon<\varepsilon$ — סתירה. לכן כל סדרה מתכנסת היא אכן סדרת קושי. $\blacksquare$
:::
...
הפיתוח העשרוני של $2.5$ הוא $2.4999\dots$, וסדרת הקירובים שלו היא $2.4,\ 2.49,\ 2.499,\ \dots$ — המתכנסת ל-$2.5$.
:::
