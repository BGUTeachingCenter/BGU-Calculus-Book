...
::: {#box-exm-factorial-seq .thmexm title="העצרת"}
...
::: {.extra title="פיבונאצ׳י, ארנבים ויחס הזהב"}
...
::: {#box-q-constant-seq .thmexr title="סדרה קבועה"}
...
::: {#box-q-one-over-n .thmexr title="הסדרה $a_n=\\frac{1}{n}$"}
...
::: {#box-q-pm-one .thmexr title="סדרה מתחלפת"}
...
::: {#box-q-one-frac-n .thmexr title="סדרה מתכנסת שאינה מונוטונית"}
...
::: {#box-q-seq-3n-4 .thmexr title="מנה של ביטויים לינאריים"}
...
### תרגילים {#sec-limit-def-exercises .unnumbered}
...
**תרגיל 9.** תהי $(a_n)_{n=1}^{\infty}$ סדרה ויהי $L\in\mathbb{R}$. הוכיחו כי אם $a_n \to L$ אז $\left|a_n\right| \to \left|L\right|$. (היעזרו ב[](04-abs-value.qmd#box-thm-reverse-triangle-inequality).)
...
בסעיף הזה נתבונן מקרוב בהגדרת הגבול, ונראה דרכים נוספות לחשוב על המושג וכן כמה תובנות מיידיות העולות ממנה.

### פירוש ״קיים $N$״ בשפה יומיומית

ננסה להציב מספר קונקרטי במקום $,\varepsilon$ ולצורך הפשטות ניקח $.\varepsilon = 1$

נניח ש-$(a_n)_{n=1}^{\infty}$ מתכנסת ל-$.8$ לפי הגדרת הגבול קיים מספר טבעי $N$ כך שלכל $n \geq N$ מתקיים $.\left|a_n - 8\right| < 1$ ננתח לפרטים מה בדיוק אומרת ההגדרה הזאת.
...
התבנית ״קיים $N$ כך שלכל $n \geq N$ מתקיים $P(n)$״, כאשר $P(n)$ היא טענה כלשהי על $,n$ חוזרת על עצמה לעיתים קרובות, ובמקום לכתוב אותה במלואה אפשר לומר ״החל ממקום מסוים״ או ״עבור $n$ גדול מספיק״.

::: {#box-key-eventually .thmkey icon="convention" title="החל ממקום מסוים"}
שלוש הצורות הבאות אומרות בדיוק את אותו הדבר: ״קיים $N \in \mathbb{N}$ כך שהתכונה מתקיימת לכל $n \geq N$״, ״התכונה מתקיימת החל ממקום מסוים״, ״התכונה מתקיימת עבור $n$ גדול מספיק״. אפשר להשתמש בכל אחת מהן, ושתי האחרונות נוחות כאשר הערך המסוים של $N$ אינו מעניין אותנו.
:::

::: {#box-warn-statement-about-n .thmwarn title="$P(n)$ היא טענה על $n$"}
בתבנית ״קיים $N$ כך שלכל $n \geq N$ מתקיים $P(n)$״, הטענה $P(n)$ היא טענה על **המספר הטבעי** $,n$ ולא על איבר הסדרה $.a_n$

למשל, $\left|a_n - 8\right| < 1$ היא טענה על $:n$ עבור כל $n$ בנפרד היא נכונה או אינה נכונה, ואנחנו שואלים החל מאיזה מקום היא נכונה תמיד. גם $a_n$ מופיע בה, אבל הוא רק חלק מן הטענה, שהרי הוא עצמו נקבע על ידי $.n$
:::

**נחזור לטענה המקורית:** קיים מספר טבעי $N$ כך שלכל $n \geq N$ מתקיים $.\left|a_n - 8\right| < 1$
...
::: {#box-key-splitting .thmkey icon="rule" title="פיצול טענות מהסוג ״החל ממקום מסוים״"}
טענה מהסוג ״החל ממקום מסוים״ שבתוכה מופיע ״וגם״ אפשר לפצל לשתי טענות מאותו סוג. במפורש: אם הטענה ״$P(n)$ **וגם** $Q(n)$״ מתקיימת החל ממקום מסוים, אז גם $P(n)$ לבדה מתקיימת החל ממקום מסוים, וגם $Q(n)$ לבדה מתקיימת החל ממקום מסוים.

אין כאן שום עבודה לעשות: המקום שהחל ממנו שתי הטענות מתקיימות יחד מתאים, כמובן, גם לכל אחת מהן בנפרד.
:::

נלך עכשיו בכיוון ההפוך. בידינו שתי הטענות הקונקרטיות שקיבלנו זה עתה: החל ממקום מסוים מתקיים $,a_n > 7$ והחל ממקום מסוים מתקיים $.a_n < 9$ האם מותר לשלב אותן ולומר שהחל ממקום מסוים מתקיימות **שתיהן** יחד, כלומר ש-$?7 < a_n < 9$

התשובה היא כן, אבל נדרשת כאן מעט זהירות, משום שכל אחת משתי הטענות מסתירה מקום משלה: הראשונה נותנת $N_1$ כך שלכל $n \geq N_1$ מתקיים $,a_n > 7$ והשנייה נותנת $N_2$ כך שלכל $n \geq N_2$ מתקיים $,a_n < 9$ ואין שום סיבה ששני המקומות יהיו שווים. אבל אין בכך מכשול: ניקח $.N = \max\left\{N_1,\ N_2\right\}$ כל $n \geq N$ מקיים גם $n \geq N_1$ וגם $,n \geq N_2$ ולכן שתי הטענות מתקיימות עבורו, והחל מהמקום $N$ מתקיים

$$.7 < a_n < 9$$

::: {#box-key-finitely-many .thmkey icon="rule" title="שילוב טענות מהסוג ״החל ממקום מסוים״"}
אפשר גם ללכת בכיוון ההפוך ולשלב שתי טענות מהסוג ״החל ממקום מסוים״ לטענה אחת מאותו סוג. במפורש: אם $P(n)$ מתקיימת החל ממקום מסוים, וגם $Q(n)$ מתקיימת החל ממקום מסוים, אז החל ממקום מסוים מתקיימות שתיהן **יחד**.

כאן, בניגוד לפיצול, יש עבודה קטנה לעשות: לכל אחת מן הטענות יש מקום משלה, ויש לבחור את המאוחר מבין השניים.

אותו טיעון בדיוק עובד עבור כל מספר **סופי** של טענות $,P_1(n),\ \dots,\ P_k(n)$ ולכן גם הן מתקיימות יחד החל ממקום מסוים.
:::

::: {#box-exm-infinitely-many-fails .thmexm title="שילוב של אינסוף טענות"}
נתבונן בסדרה $.a_n = \dfrac{1}{n}$ לכל $k$ טבעי מתקיים $\dfrac{1}{n} < \dfrac{1}{k}$ לכל $,n > k$ ולכן הטענה $a_n < \dfrac{1}{k}$ מתקיימת החל ממקום מסוים. אלה אינסוף טענות, אחת לכל $.k$

אילו היה מותר לשלב את כולן, היינו מקבלים שהחל ממקום מסוים מתקיים $a_n < \dfrac{1}{k}$ **לכל** $,k$ ומכאן $.a_n \leq 0$ אבל כל איברי הסדרה חיוביים, ולכן זה אינו נכון.

**מה השתבש כאן?** נחזור אל הטיעון של השילוב. לכל $k$ הטענה $a_n < \dfrac{1}{k}$ אכן מתקיימת, אבל היא מתקיימת החל ממקום **משלה**, והמקום הזה הוא $.n = k+1$ ככל ש-$k$ גדל, המקום מתרחק.

השילוב של שתי טענות התבסס על לקיחת המאוחר מבין שני המקומות. כדי לשלב את כל אינסוף הטענות היינו צריכים לקחת את המאוחר מבין אינסוף המקומות $,2,\ 3,\ 4,\ \dots$ ולקבוצה אינסופית כזאת אין איבר גדול ביותר. בדיוק שם נשבר הטיעון.
:::

::: {#box-warn-finiteness-essential .thmwarn title="הסופיות חיונית"}
השילוב דורש מספר **סופי** של טענות. כאשר מספר הטענות אינו סופי הוא עלול להיכשל.
:::

גם הפיצול אינו חף מהפתעות, והפעם הקושי אינו במספר הטענות אלא במילה המחברת ביניהן.

::: {#box-exm-or-fails .thmexm title="פיצול של טענה המחוברת ב״או״"}
נתבונן בסדרה $.a_n = \left(-1\right)^{n}$ החל ממקום מסוים מתקיים $a_n = 1$ **או** $,a_n = -1$ שהרי הדבר נכון לכל $.n$

ובכל זאת אין זה נכון שהחל ממקום מסוים מתקיים $,a_n = 1$ ואין זה נכון שהחל ממקום מסוים מתקיים $:a_n = -1$ שתי האפשרויות מתחלפות ביניהן לכל אורך הסדרה.

**מה השתבש כאן?** בפיצול של ״וגם״ הסתמכנו על כך שאותו מקום עצמו מתאים לשתי הטענות. ב״או״ אין מקום כזה כלל: בכל מקום מתקיימת אחת משתי האפשרויות, אבל לא אותה אחת כל הזמן, ולכן אין אף אחת מהן שמתקיימת החל ממקום מסוים.
:::

::: {#box-warn-and-not-or .thmwarn title="הפיצול עובד עם ״וגם״, לא עם ״או״"}
פיצול של טענה מהסוג ״החל ממקום מסוים״ מותר כאשר המילה המחברת היא **״וגם״**. כאשר המילה המחברת היא **״או״** הפיצול אינו מותר.
:::

<!-- TODO (לא עכשיו): הכישלון של ״או״ כאן הוא מקרה פרטי של כך ש-∀ אינו מתפלג מעל ∨.
     ראוי להרחבה קצרה בפרק הלוגיקה (01-logic.qmd), ואז אפשר להפנות לכאן משם. -->

### טווח ערכי הסדרה החל ממקום מסוים

בתת-הפרק הקודם ראינו שאם הסדרה מתכנסת ל-$8$ וניקח $\varepsilon = 1$ בהגדרה, הדבר מתורגם לכך שהחל ממקום מסוים מתקיים $.7 < a_n < 9$

באותו אופן, אם נחזור על אותם צעדים עם $,\varepsilon = 0.1$ נקבל שהחל ממקום מסוים מתקיים $.7.9 < a_n < 8.1$

נשים לב שזה עובד לכל $,\varepsilon$ ולכל גבול $,L$ ולאו דווקא עבור $.8$ אם נסמן את $L + \varepsilon$ על ידי $M$ ואת $L - \varepsilon$ על ידי $,m$ נקבל את המשפט הבא.
...
::: {#box-exr-window-characterisation .thmexr}
הוכיחו כי $\lim\limits_{n\to\infty} a_n = L$ **אם ורק אם** לכל $m$ ו-$M$ המקיימים $m < L < M$ מתקיים $m < a_n < M$ החל ממקום מסוים.
:::

::: {.thmsol}
**הכיוון הראשון.** נניח $,\lim\limits_{n\to\infty} a_n = L$ ויהיו $m$ ו-$M$ המקיימים $.m < L < M$ אפשר להפעיל עליהם את [](#box-thm-limit-window):

סעיף (א) נותן $N_1 \in \mathbb{N}$ כך שלכל $n \geq N_1$ מתקיים $.a_n > m$

סעיף (ב) נותן $N_2 \in \mathbb{N}$ כך שלכל $n \geq N_2$ מתקיים $.a_n < M$

נבחר $.N = \max\left\{N_1,\ N_2\right\}$ אז לכל $n \geq N$ שני האי-שוויונות מתקיימים, כלומר $.m < a_n < M$

**הכיוון השני.** נניח שהתנאי מתקיים לכל זוג מספרים $m$ ו-$M$ כאלה, ויהי $.\varepsilon > 0$

המספרים $m = L - \varepsilon$ ו-$M = L + \varepsilon$ מקיימים $,m < L < M$ ולכן ההנחה חלה עליהם: החל ממקום מסוים מתקיים
...
בתת-הפרק הקודם ראינו כיצד הגבול מכתיב את ערכי הסדרה: בהינתן $,L$ ידענו לומר היכן נמצאים איברי הסדרה החל ממקום מסוים. טבעי לשאול את השאלה בכיוון ההפוך, כלומר כיצד ערכי האיברים מכתיבים את הגבול.

למשל, נניח ש-$(a_n)_{n=1}^{\infty}$ סדרה מתכנסת שכל איבריה חיוביים. האם בהכרח גם הגבול שלה חיובי? נחזור לשאלה הזאת מיד.

התשובה טמונה במשפט הבא.
...
::: {.thmproof title="סעיף (א)"}
נניח בשלילה ש-$.L < m$ אז $m$ הוא מספר הגדול מן הגבול, ולכן לפי סעיף (ב) של [](#box-thm-limit-window) קיים $N \in \mathbb{N}$ כך שלכל $n \geq N$ מתקיים $.a_n < m$ בפרט $,a_N < m$ בסתירה להנחה ש-$a_n \geq m$ לכל $.n$ לכן $.L \geq m$
:::
...
נחזור עכשיו לשאלה ששאלנו. כל איברי הסדרה מקיימים $,a_n > 0$ ובפרט $,a_n \geq 0$ ולכן סעיף (א) של המשפט נותן $.L \geq 0$ אבל זה כל מה שהוא נותן, והתשובה לשאלה כפי ששאלנו אותה היא **לא**: ניקח את הסדרה $,a_n = \dfrac{1}{n}$ שכל איבריה חיוביים, ואילו הגבול שלה הוא $,0$ שאינו חיובי.

לעומת זאת, אילו שאלנו האם סדרה שכל איבריה **אי-שליליים** מתכנסת לגבול אי-שלילי, התשובה הייתה כן, וזהו בדיוק סעיף (א) של המשפט.
...
# The strict-to-weak picture: every term sits STRICTLY above the line, and the limit
# sits exactly ON it. One frozen frame is the whole point, so this is a static figure
# and not a widget — there is no parameter worth dragging.
n = np.arange(1, 26)
a = 1.0 / n

fig, ax = plt.subplots(figsize=(6.4, 3.0))
ax.axhline(0, color="tab:red", lw=1.6, ls="--")
ax.plot(n, a, "o", ms=5, color="tab:blue", zorder=3)
# the limit itself, on the line the terms never reach
ax.plot([25.8], [0], "o", ms=9, color="tab:red", zorder=4, clip_on=False)

ax.annotate(r"$a_n = \frac{1}{n} > 0$", xy=(9, 0.42), fontsize=12, color="tab:blue")
ax.annotate(r"$L = 0$", xy=(23.0, 0.075), fontsize=12, color="tab:red")
...
ax.spines[["top", "right"]].set_visible(False)
fig.savefig("figures/c07_fig18.png", dpi=150, bbox_inches="tight")
plt.close(fig)
```

```{=latex}
\par\medskip
\noindent\beginL\hbox to \linewidth{\hss\includegraphics[width=0.62\linewidth]{figures/c07_fig18.png}\hss}\endL\par
\medskip
```

::: {.content-visible when-format="pdf" style="text-align:center"}
תרשים: כל איברי הסדרה $a_n = \frac{1}{n}$ נמצאים **מעל** הקו $,y = 0$ ואף אחד מהם אינו נוגע בו; ובכל זאת הגבול יושב בדיוק עליו. כך אי-שוויון חד על האיברים הופך לאי-שוויון חלש על הגבול.
:::

::: {.content-visible when-format="html"}
![כל איברי הסדרה $a_n = \frac{1}{n}$ נמצאים **מעל** הקו $,y = 0$ ואף אחד מהם אינו נוגע בו; ובכל זאת הגבול יושב בדיוק עליו. כך אי-שוויון חד על האיברים הופך לאי-שוויון חלש על הגבול.](figures/c07_fig18.png){#fig-strict-becomes-weak width="62%" fig-align="center"}
:::
...
מתקיים $,L_1 < M < L_2$ ולכן אפשר להפעיל את [](#box-thm-limit-window) פעמיים, פעם על כל גבול:
...
**תרגיל 1.** כל אחת מן הסדרות הבאות מתכנסת, והגבול שלה הוא אחד מן הערכים המוצעים. מצאו אותו. אין צורך בחישוב: חסמו את הסדרה, העבירו את החסמים לגבול, וראו איזה מן הערכים שורד.

(א) $,a_n = \sin\left(\sqrt{n+1} - \sqrt{n}\right)$ והערכים המוצעים $.-3,\ 0,\ 2,\ 5$

::: {.content-visible when-format="html"}
```{=html}
<div class="tf-quiz" data-correct="b">
<button class="tf-opt" data-val="a">\(-3\)</button>
<button class="tf-opt" data-val="b">\(0\)</button>
<button class="tf-opt" data-val="c">\(2\)</button>
<button class="tf-opt" data-val="d">\(5\)</button>
<div class="tf-feedback"></div>
</div>
```
:::

(ב) $,a_n = 5 + \dfrac{\cos\left(n!\right)}{\sqrt{n}}$ והערכים המוצעים $.0,\ 5,\ 8,\ 10$

::: {.content-visible when-format="html"}
```{=html}
<div class="tf-quiz" data-correct="b">
<button class="tf-opt" data-val="a">\(0\)</button>
<button class="tf-opt" data-val="b">\(5\)</button>
<button class="tf-opt" data-val="c">\(8\)</button>
<button class="tf-opt" data-val="d">\(10\)</button>
<div class="tf-feedback"></div>
</div>
```
:::

(ג) $,a_n = \dfrac{2n + \sin\left(n^{3}\right)}{n}$ והערכים המוצעים $.-1,\ 0,\ 2,\ 7$

::: {.content-visible when-format="html"}
```{=html}
<div class="tf-quiz" data-correct="c">
<button class="tf-opt" data-val="a">\(-1\)</button>
<button class="tf-opt" data-val="b">\(0\)</button>
<button class="tf-opt" data-val="c">\(2\)</button>
<button class="tf-opt" data-val="d">\(7\)</button>
<div class="tf-feedback"></div>
</div>
```
:::

(ד) $,a_n = 7 - \dfrac{3\cos\left(2^{n}\right)}{n}$ והערכים המוצעים $.0,\ 3,\ 7,\ 12$

::: {.content-visible when-format="html"}
```{=html}
<div class="tf-quiz" data-correct="c">
<button class="tf-opt" data-val="a">\(0\)</button>
<button class="tf-opt" data-val="b">\(3\)</button>
<button class="tf-opt" data-val="c">\(7\)</button>
<button class="tf-opt" data-val="d">\(12\)</button>
<div class="tf-feedback"></div>
</div>
```
:::
...
::: {.thmsol .foldable}
**סעיף (א).** לכל $x$ ממשי מתקיים $,-1 \leq \sin x \leq 1$ ולכן $-1 \leq a_n \leq 1$ לכל $,n$ ולפי [](#box-thm-limit-preserves-order) גם $.-1 \leq L \leq 1$ מבין הערכים המוצעים רק $0$ נמצא בקטע הזה, ולכן הגבול הוא $.0$

**סעיף (ב).** לכל $n$ מתקיים $,\left|\dfrac{\cos\left(n!\right)}{\sqrt{n}}\right| \leq \dfrac{1}{\sqrt{n}} \leq 1$ ולכן $,4 \leq a_n \leq 6$ ומכאן $.4 \leq L \leq 6$ רק $5$ נמצא בקטע, ולכן הגבול הוא $.5$

**סעיף (ג).** נחלק את המונה אבר-אבר:

$$.a_n = \frac{2n}{n} + \frac{\sin\left(n^{3}\right)}{n} = 2 + \frac{\sin\left(n^{3}\right)}{n}$$

לכל $n$ מתקיים $,\left|\dfrac{\sin\left(n^{3}\right)}{n}\right| \leq \dfrac{1}{n} \leq 1$ ולכן $,1 \leq a_n \leq 3$ ומכאן $.1 \leq L \leq 3$ רק $2$ נמצא בקטע, ולכן הגבול הוא $.2$

**סעיף (ד).** לכל $n$ מתקיים $,\left|\dfrac{3\cos\left(2^{n}\right)}{n}\right| \leq \dfrac{3}{n} \leq 3$ ולכן $,4 \leq a_n \leq 10$ ומכאן $.4 \leq L \leq 10$ רק $7$ נמצא בקטע, ולכן הגבול הוא $.7$

בכל ארבעת הסעיפים לא חישבנו דבר. חסמנו את הסדרה בעזרת $\left|\sin x\right| \leq 1$ או $,\left|\cos x\right| \leq 1$ העברנו את החסמים לגבול לפי המשפט, וראינו שרק ערך אחד מן הרשימה שורד.
:::

**תרגיל 2.** תהי $(a_n)_{n=1}^{\infty}$ סדרה מתכנסת שכל איבריה מקיימים $.2 < a_n < 4$ אילו מן המספרים $1,\ 2,\ 2.5,\ 4,\ 4.0001,\ 5$ **אינם יכולים** להיות הגבול שלה?

::: {.content-visible when-format="html"}
```{=html}
<div class="set-quiz" data-answer="1,4.0001,5"><span class="set-label">המספרים שנפסלים</span><input class="set-input" type="text" placeholder="לדוגמה: 1, 2.5"><button class="set-check">בדיקה</button><span class="set-feedback"></span></div>
```
:::

::: {.thmsol .foldable}
מ-$2 < a_n < 4$ נובע בפרט $2 \leq a_n \leq 4$ לכל $,n$ ולכן לפי [](#box-thm-limit-preserves-order) מתקיים $.2 \leq L \leq 4$

המספרים $1$, $4.0001$ ו-$5$ אינם בקטע $,\left[2,\ 4\right]$ ולכן אינם יכולים להיות הגבול. שלושת הנותרים אינם נפסלים.
:::

**תרגיל 3.** תהי $(b_n)_{n=1}^{\infty}$ סדרה מתכנסת שכל איבריה מקיימים $.-3 \leq b_n < 1$ אילו מן המספרים $-4,\ -3,\ 0,\ 1,\ 2$ **יכולים** להיות הגבול שלה?

::: {.content-visible when-format="html"}
```{=html}
<div class="set-quiz" data-answer="-3,0,1"><span class="set-label">המספרים האפשריים</span><input class="set-input" type="text" placeholder="לדוגמה: 0, 1"><button class="set-check">בדיקה</button><span class="set-feedback"></span></div>
```
:::

::: {.thmsol .foldable}
מן הנתון נובע בפרט $-3 \leq b_n \leq 1$ לכל $,n$ ולכן לפי [](#box-thm-limit-preserves-order) מתקיים $.-3 \leq L \leq 1$

לכן $-4$ ו-$2$ אינם יכולים להיות הגבול, ואילו $-3$, $0$ ו-$1$ יכולים.

שימו לב ש-$1$ יכול להיות הגבול, אף שכל איברי הסדרה מקיימים $b_n < 1$ ממש. זו אותה תופעה שבתיבה שלמעלה.
:::


## פיתוח עשרוני כסדרה (העשרה) {.unnumbered}
