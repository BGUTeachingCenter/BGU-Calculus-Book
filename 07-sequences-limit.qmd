...
**תרגיל 8.** ידוע כי $2^{n} \geq n$ לכל $n\in\mathbb{N}$ (טענה שמוכיחים [באינדוקציה](05-induction.qmd)). הוכיחו כי $\lim\limits_{n\to\infty} \dfrac{1}{2^{n}} = 0$.
...
אמרנו, שבאופן אינטואיטיבי, סדרה היא מתכנסת אם היא ״מקרבת״ מספר כלשהו. ברור שאם ״סדרה מקרבת״ מספר, היא צריכה לקרב מספר יחיד ולא כמה מספרים. כמו שאמרנו קודם, אחד התפקידים של ההגדרה הוא לתת מסגרת מוגדרת ומדויקת לאינטואיציה שלנו. עם זאת, ההגדרה הפורמלית של הגבול אינה דורשת יחידות: היא קובעת שהמספר $L$ הוא גבול של הסדרה אם לכל $\varepsilon>0$ קיים $N\in\mathbb{N}$ כך שלכל $n \geq N$ מתקיים $,|a_n - L| < \varepsilon$ ואין בה דבר שמונע ממספר אחר לקיים בדיוק את אותו תנאי.

מִנַּיִן לנו שלא יכולים להיות כמה גבולות במקרים ״מוזרים״ מסוימים? (כפי שתראו בהמשך, במתמטיקה יש לא מעט כאלה.) לא היינו רוצים שתהיה אפשרות שהקירוב שלנו יקרב מספרים שונים.

אפשר לנסות לפתור את הבעיה בהגדרה ופשוט לדרוש יחידות, אך זה לא באמת פותר אותה. קודם כל, איך להתייחס למקרי ביניים שבהם יש כמה מספרים שמקיימים את ההגדרה של הגבול? ודבר שני — איך אפשר בכלל להבטיח יחידות? ברגע שמצאנו מספר אחד, איך בודקים שאין אחרים? לכן, **לדרוש** יחידות כחלק מההגדרה אינו עוזר לנו בהרבה. על מנת לבדוק ״ייתכנות״ של ההגדרה, צריך לענות על השאלה: האם יכולים להיות לסדרה שני גבולות שונים?

מסתבר, שמתוך ההגדרה נובע שלא יכול להיות יותר מגבול אחד — וזה מנוסח במשפט הבא.
...
::::::::::: {#box-expl-uniqueness-idea .thmexpl title="הרעיון של ההוכחה"}
נניח ש-$,L_1 \neq L_2$ ונסמן ב-$d$ את המרחק ביניהם, כלומר $.d = |L_1 - L_2| > 0$

**הרעיון:** אם המרחק בין $x$ ל-$L_1$ קטן מ-$\frac{d}{2}$, אז המרחק בין $x$ ל-$L_2$ גדול מ-$\frac{d}{2}$; ובאותו אופן, אם המרחק בין $x$ ל-$L_2$ קטן מ-$\frac{d}{2}$, אז המרחק בין $x$ ל-$L_1$ גדול מ-$\frac{d}{2}$. במילים אחרות, **נקודה אחת אינה יכולה להיות קרובה מ-**$\frac{d}{2}$ גם ל-$L_1$ וגם ל-$L_2$.

זוהי תוצאה מיידית של [אי-שוויון המשולש](04-abs-value.qmd): אילו היו שני המרחקים קטנים מ-$\frac{d}{2}$, היינו מקבלים $$.d = |L_1 - L_2| \leq |x-L_1| + |x-L_2| < \frac{d}{2} + \frac{d}{2} = d$$ כלומר $,d < d$ וזה כמובן בלתי אפשרי.
...
L1, L2 = -1.0, 1.0
d = L2 - L1
h = d / 2.0                 # the neighbourhood radius, d/2
m = (L1 + L2) / 2.0

# the two d/2 neighbourhoods: they meet exactly at the midpoint and do not overlap
ax.plot([L1-h, L1+h], [0, 0], color="tab:blue", lw=7, alpha=0.30, zorder=2,
        solid_capstyle="butt")
ax.plot([L2-h, L2+h], [0, 0], color="tab:red", lw=7, alpha=0.30, zorder=2,
        solid_capstyle="butt")

for p, lab, col in [(L1, r"$L_1$", "tab:blue"), (L2, r"$L_2$", "tab:red")]:
    ax.plot([p], [0], "o", color=col, ms=7, zorder=5)
    ax.annotate(lab, xy=(p, 0), xytext=(p, -0.16), ha="center", va="top",
                fontsize=13, color=col)

# midpoint tick: the only place the two neighbourhoods touch
ax.plot([m, m], [-0.10, 0.10], color="black", lw=1.4, zorder=5)

# the radius of each neighbourhood, measured on the OUTER side: it keeps the middle
# clear, where the two bands meeting at the midpoint is what should draw the eye
ax.annotate("", xy=(L1, 0.36), xytext=(L1-h, 0.36),
            arrowprops=dict(arrowstyle="<->", color="tab:blue", lw=1.2))
ax.annotate(r"$\frac{d}{2}$", xy=(L1-h/2, 0.36), xytext=(L1-h/2, 0.42),
            ha="center", fontsize=12, color="tab:blue")
ax.annotate("", xy=(L2, 0.36), xytext=(L2+h, 0.36),
            arrowprops=dict(arrowstyle="<->", color="tab:red", lw=1.2))
ax.annotate(r"$\frac{d}{2}$", xy=(L2+h/2, 0.36), xytext=(L2+h/2, 0.42),
            ha="center", fontsize=12, color="tab:red")

# d across the top, with dotted guides down to the points as in the case figures
ax.annotate("", xy=(L1, 0.84), xytext=(L2, 0.84),
            arrowprops=dict(arrowstyle="<->", color="black", lw=1.3))
ax.annotate(r"$d$", xy=(m, 0.84), xytext=(m, 0.90), ha="center", fontsize=12)
for p in (L1, L2):
    ax.plot([p, p], [0.10, 0.84], color="black", lw=0.7, ls=":")

# one point in each neighbourhood, and one in neither
for p, lab in [(-1.45, r"$x_1$"), (0.55, r"$x_2$"), (2.6, r"$x_3$")]:
    ax.plot([p], [0], "o", color="tab:green", ms=6, zorder=6)
    ax.annotate(lab, xy=(p, 0), xytext=(p, 0.13), ha="center", fontsize=11,
                color="tab:green")
...
:::::::: {#box-sim-two-neighbourhoods .thmsim title="שתי הסביבות ברדיוס $\\frac{d}{2}$"}
::: {.content-visible when-format="html"}
```{=html}
<iframe class="widget-frame" src="widgets/ch07/two-limits-neighbourhoods.html" loading="lazy" title="שתי סביבות ברדיוס d/2 סביב שני גבולות, ונקודה נעה ביניהן"></iframe>
```
:::

הזיזו את $x$ לאורך הישר, ונסו למצוא מיקום שבו $x$ נמצאת בשתי הסביבות בו-זמנית. לא תמצאו: בכל מיקום, לפחות אחד משני המרחקים גדול מ-$\frac{d}{2}$. שימו לב גם לשוויון המוצג מעל, המשתנה לפי מיקומה של $x$, והוא בדיוק אחד משלושת המקרים שנראה מיד.
...
::: {style="text-align:center"}
תרשים: הסביבות ברדיוס $\frac{d}{2}$ סביב $L_1$ ו-$L_2$ נפגשות בדיוק באמצע ואינן חופפות. הנקודה $x_1$ נמצאת בסביבה של $L_1$ בלבד, $x_2$ בסביבה של $L_2$ בלבד, ו-$x_3$ מחוץ לשתיהן
:::
::::
::::::::

**שימו לב לאי-השוויון החד.** הסביבות מוגדרות באמצעות אי-שוויון חד: הסביבה של $L_1$ היא אוסף הנקודות המקיימות $,|x - L_1| < \frac{d}{2}$ ולא $.|x - L_1| \leq \frac{d}{2}$ ההבדל מתגלה בדיוק באמצע הקטע: המרחק משם אל $L_1$ ואל $L_2$ שווה $\frac{d}{2}$ בשני המקרים, ולכן נקודת האמצע אינה שייכת לאף אחת משתי הסביבות. טעות נפוצה היא לחשוב שנקודת האמצע היא ״החיתוך״ שלהן, אך החיתוך **ריק**: אין ולו נקודה אחת השייכת לשתיהן.

זה אינו פרט טכני בלבד. בהגדרת הגבול מופיע $|a_n - L| < \varepsilon$ עם אי-שוויון חד, ובזכותו הבחירה $\varepsilon = \frac{d}{2}$ מספיקה. אילו ההגדרה הייתה מסתפקת באי-שוויון לא חד, היינו מקבלים $,d \leq d$ וזו אינה סתירה, והיה עלינו לבחור $\varepsilon$ קטן יותר, למשל $.\frac{d}{3}$

כדי לראות מדוע סכום שני המרחקים אינו קטן מ-$d$, נסתכל על שלושת המיקומים האפשריים של $x$ ביחס ל-$L_1$ ול-$L_2$.

**המקרה הראשון:** $x$ נמצא בקטע המחבר את $L_1$ ו-$L_2$. אז סכום המרחקים בין $x$ ל-$L_1$ ובין $x$ ל-$L_2$ הוא בדיוק המרחק בין $L_1$ ל-$L_2$. כלומר: $$
.|x-L_1|+|x-L_2| = |L_1 - L_2|
$$

כאן מתקיים שוויון, ולכן שני המרחקים משלימים זה את זה ל-$d$: אם $|x-L_1| < \frac{d}{2}$, אז $$.|x-L_2| = d - |x-L_1| > d - \frac{d}{2} = \frac{d}{2}$$ ובאותו אופן עם החלפת התפקידים של $L_1$ ו-$L_2$.
...
L1, L2, xp = -2.0, 2.0, 0.6

for p, lab, col in [(L1, r"$L_1$", "tab:blue"), (L2, r"$L_2$", "tab:red"), (xp, r"$x$", "tab:green")]:
    ax.plot([p], [0], "o", color=col, ms=7, zorder=4)
    ax.annotate(lab, xy=(p, 0), xytext=(p, -0.13), ha="center", va="top", fontsize=13, color=col)

# the two distances from x, just above the line
ax.annotate("", xy=(L1, 0.30), xytext=(xp, 0.30),
            arrowprops=dict(arrowstyle="<->", color="tab:blue", lw=1.3))
ax.annotate(r"$|x-L_1|$", xy=((L1+xp)/2, 0.30), xytext=((L1+xp)/2, 0.36),
            ha="center", fontsize=11, color="tab:blue")

ax.annotate("", xy=(xp, 0.30), xytext=(L2, 0.30),
            arrowprops=dict(arrowstyle="<->", color="tab:red", lw=1.3))
ax.annotate(r"$|x-L_2|$", xy=((xp+L2)/2, 0.30), xytext=((xp+L2)/2, 0.36),
            ha="center", fontsize=11, color="tab:red")

# total distance d
ax.annotate("", xy=(L1, 0.80), xytext=(L2, 0.80),
            arrowprops=dict(arrowstyle="<->", color="black", lw=1.3))
ax.annotate(r"$d=|L_1-L_2|$", xy=(0, 0.80), xytext=(0, 0.86), ha="center", fontsize=12)

for p in (L1, L2):
    ax.plot([p, p], [0.05, 0.80], color="black", lw=0.7, ls=":")
ax.plot([xp, xp], [0.05, 0.30], color="black", lw=0.7, ls=":")
...
::: {style="text-align:center"}
תרשים: כאשר $x$ נמצא בקטע שבין $L_1$ ל-$L_2$, סכום שני המרחקים שווה בדיוק ל-$d$
:::

::: {.content-visible when-format="html"}
![כאשר $x$ נמצא בקטע שבין $L_1$ ל-$L_2$, סכום שני המרחקים שווה בדיוק ל-$d$](figures/c07_fig16.png){#fig-uniq-case-between width="62%" fig-align="center"}
:::

**המקרה השני:** $x$ נמצא מחוץ לקטע, מעברו השני של $L_1$. אז המרחק בין $x$ ל-$L_2$ הוא סכום המרחק בין $x$ ל-$L_1$ והמרחק בין $L_1$ ל-$L_2$. כלומר: $$
.|x-L_1|+|L_1-L_2| = |x - L_2|
$$

כאן המרחק אל $L_2$ אינו קטן מ-$d$ באף מצב, שכן $,|x-L_2| = |x-L_1| + d \geq d$ ובפרט הוא גדול מ-$\frac{d}{2}$. כלומר, גם אם $x$ קרוב מאוד ל-$L_1$, הוא רחוק מ-$L_2$ יותר מ-$\frac{d}{2}$.
...
L1, L2, xp = -0.8, 2.2, -2.8

for p, lab, col in [(L1, r"$L_1$", "tab:blue"), (L2, r"$L_2$", "tab:red"), (xp, r"$x$", "tab:green")]:
    ax.plot([p], [0], "o", color=col, ms=7, zorder=4)
    ax.annotate(lab, xy=(p, 0), xytext=(p, -0.13), ha="center", va="top", fontsize=13, color=col)

# |x - L1|, then d, laid end to end on the lower level
ax.annotate("", xy=(xp, 0.30), xytext=(L1, 0.30),
            arrowprops=dict(arrowstyle="<->", color="tab:green", lw=1.3))
ax.annotate(r"$|x-L_1|$", xy=((xp+L1)/2, 0.30), xytext=((xp+L1)/2, 0.36),
            ha="center", fontsize=11, color="tab:green")

ax.annotate("", xy=(L1, 0.30), xytext=(L2, 0.30),
            arrowprops=dict(arrowstyle="<->", color="black", lw=1.3))
ax.annotate(r"$d$", xy=((L1+L2)/2, 0.30), xytext=((L1+L2)/2, 0.36),
            ha="center", fontsize=12)

# their sum is the distance from x to L2
ax.annotate("", xy=(xp, 0.82), xytext=(L2, 0.82),
            arrowprops=dict(arrowstyle="<->", color="tab:red", lw=1.3))
ax.annotate(r"$|x-L_2|$", xy=((xp+L2)/2, 0.82), xytext=((xp+L2)/2, 0.88),
            ha="center", fontsize=11, color="tab:red")

for p in (xp, L1, L2):
    ax.plot([p, p], [0.05, 0.30], color="black", lw=0.7, ls=":")
for p in (xp, L2):
    ax.plot([p, p], [0.30, 0.82], color="black", lw=0.7, ls=":")
...
```{=latex}
\par\medskip
\noindent\beginL\hbox to \linewidth{\hss\includegraphics[width=0.62\linewidth]{figures/c07_fig17.png}\hss}\endL\par
\medskip
```

::: {style="text-align:center"}
תרשים: כאשר $x$ נמצא מעברו השני של $L_1$, המרחק אל $L_2$ הוא סכום המרחק אל $L_1$ ואורך הקטע $d$
:::

::: {.content-visible when-format="html"}
![כאשר $x$ נמצא מעברו השני של $L_1$, המרחק אל $L_2$ הוא סכום המרחק אל $L_1$ ואורך הקטע $d$](figures/c07_fig17.png){#fig-uniq-case-left width="62%" fig-align="center"}
:::

**המקרה השלישי:** $x$ נמצא מחוץ לקטע, מעברו השני של $L_2$. אז המרחק בין $x$ ל-$L_1$ הוא סכום המרחק בין $x$ ל-$L_2$ והמרחק בין $L_1$ ל-$L_2$. כלומר: $$
.|x-L_2| + |L_1 - L_2| = |x - L_1|
$$

כאן, באופן סימטרי, $,|x-L_1| = |x-L_2| + d \geq d$ ולכן המרחק אל $L_1$ גדול מ-$\frac{d}{2}$, יהיה $x$ קרוב ל-$L_2$ ככל שיהיה.
...
L1, L2, xp = -2.2, 0.8, 2.8

for p, lab, col in [(L1, r"$L_1$", "tab:blue"), (L2, r"$L_2$", "tab:red"), (xp, r"$x$", "tab:green")]:
    ax.plot([p], [0], "o", color=col, ms=7, zorder=4)
    ax.annotate(lab, xy=(p, 0), xytext=(p, -0.13), ha="center", va="top", fontsize=13, color=col)

# d, then |x - L2|, laid end to end on the lower level
ax.annotate("", xy=(L1, 0.30), xytext=(L2, 0.30),
            arrowprops=dict(arrowstyle="<->", color="black", lw=1.3))
ax.annotate(r"$d$", xy=((L1+L2)/2, 0.30), xytext=((L1+L2)/2, 0.36),
            ha="center", fontsize=12)

ax.annotate("", xy=(L2, 0.30), xytext=(xp, 0.30),
            arrowprops=dict(arrowstyle="<->", color="tab:green", lw=1.3))
ax.annotate(r"$|x-L_2|$", xy=((L2+xp)/2, 0.30), xytext=((L2+xp)/2, 0.36),
            ha="center", fontsize=11, color="tab:green")

# their sum is the distance from x to L1
ax.annotate("", xy=(L1, 0.82), xytext=(xp, 0.82),
            arrowprops=dict(arrowstyle="<->", color="tab:blue", lw=1.3))
ax.annotate(r"$|x-L_1|$", xy=((L1+xp)/2, 0.82), xytext=((L1+xp)/2, 0.88),
            ha="center", fontsize=11, color="tab:blue")
...
::: {style="text-align:center"}
תרשים: כאשר $x$ נמצא מעברו השני של $L_2$, המרחק אל $L_1$ הוא סכום המרחק אל $L_2$ ואורך הקטע $d$
:::

::: {.content-visible when-format="html"}
![כאשר $x$ נמצא מעברו השני של $L_2$, המרחק אל $L_1$ הוא סכום המרחק אל $L_2$ ואורך הקטע $d$](figures/c07_fig18.png){#fig-uniq-case-right width="62%" fig-align="center"}
:::

**בשלושת המקרים** סכום המרחקים מ-$x$ אל $L_1$ ואל $L_2$ אינו קטן מן המרחק שבין $L_1$ ל-$L_2$: במקרה הראשון הוא שווה לו בדיוק, ובשני האחרים הוא גדול ממנו. זהו בדיוק אי-שוויון המשולש: $$.|x-L_1| + |x-L_2| \geq |L_1 - L_2| = d$$

**ההוכחה שלהלן היא בדיוק הרעיון הזה בכתיב פורמלי:** אם הסדרה מתכנסת לשני הגבולות, אז החל ממקום מסוים איבריה חייבים להיות קרובים לשניהם בו-זמנית, וזה בלתי אפשרי.
:::::::::::

::: thmproof
נניח בשלילה ש-$,L_1 \neq L_2$ ונסמן $.d = |L_1 - L_2| > 0$ נבחר $.\varepsilon = \frac{d}{2} > 0$

מכיוון ש-$,a_n \to L_1$ קיים $N_1\in\mathbb{N}$ כך שלכל $n \geq N_1$ מתקיים $.\left|a_n - L_1\right| < \varepsilon$

מכיוון ש-$,a_n \to L_2$ קיים $N_2\in\mathbb{N}$ כך שלכל $n \geq N_2$ מתקיים $.\left|a_n - L_2\right| < \varepsilon$

נבחר $,N = \max\left\{N_1,N_2\right\}$ ואז לכל $n \geq N$ שני אי-השוויונות מתקיימים בו-זמנית, ולפי אי-שוויון המשולש $$.d = |L_1 - L_2| \leq \left|a_n - L_1\right| + \left|a_n - L_2\right| < \varepsilon + \varepsilon = d$$

קיבלנו $,d < d$ וזו סתירה. לכן ההנחה $L_1 \neq L_2$ אינה יכולה להתקיים, ומכאן $.L_1 = L_2$
:::
...
ax.legend()
fig.tight_layout()
fig.savefig("figures/c07_fig19.png", dpi=150, bbox_inches="tight")
plt.close(fig)
```

```{=latex}
\par\medskip
\noindent\beginL\hbox to \linewidth{\hss\includegraphics[width=0.85\linewidth]{figures/c07_fig19.png}\hss}\endL\par
\medskip
```
