...
**תרגיל 8.** ידוע כי $2^{n} \geq n$ לכל $n\in\mathbb{N}$ (טענה שמוכיחים באינדוקציה — ראו [שאלה 5.2.2](05-induction.qmd#box-qst-induction-2n-geq-n)). הוכיחו כי $\lim\limits_{n\to\infty} \dfrac{1}{2^{n}} = 0$.
...
### מיקום איברים ביחס לגבול

המסקנה המיידית של הגדרת התכנסות, שנובעת פשוט מפתיחת הערך המוחלט שבהגדרה, היא שכמעט כל איברי הסדרה לא יכולים להיות הרבה יותר גדולים או הרבה יותר קטנים מהגבול, כפי שמנוסח במשפט הבא.
...
::: {.thmexpl title="מאיחוד שני הסעיפים אל קטע פתוח כלשהו"}
אפשר לאחד את שני הסעיפים של המשפט לטענה אחת: לכל $m$ ו-$M$ המקיימים $,m < L < M$ מתקיים

$$m < a_n < M$$

עבור $n$ גדול מספיק.

נשים לב עכשיו שהקבוצה $\left\{x \mid m < x < M\right\}$ אינה אלא הקטע הפתוח $,\left(m,\ M\right)$ והוא מכיל את $.L$ מצד שני, גם סביבת $\varepsilon$ של $,L$ כלומר $,\left(L-\varepsilon,\ L+\varepsilon\right)$ היא קטע פתוח המכיל את $.L$ מכאן המסקנה הבאה.
:::

::: {#box-cor-limit-open-interval .thmcor}
תהי $(a_n)_{n=1}^{\infty}$ סדרה ויהי $.L \in \mathbb{R}$ אז $\lim\limits_{n\to\infty} a_n = L$ אם ורק אם לכל קטע פתוח $\left(t,\ s\right)$ המקיים $L \in \left(t,\ s\right)$ קיים $N \in \mathbb{N}$ כך שלכל $n \geq N$ מתקיים $.a_n \in \left(t,\ s\right)$
:::
...
השלימו את פרטי ההוכחה של המסקנה: הוכיחו כי $\lim\limits_{n\to\infty} a_n = L$ אם ורק אם לכל קטע פתוח $\left(t,\ s\right)$ המקיים $L \in \left(t,\ s\right)$ קיים $N \in \mathbb{N}$ כך שלכל $n \geq N$ מתקיים $.a_n \in \left(t,\ s\right)$
:::
