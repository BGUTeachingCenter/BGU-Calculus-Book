...
::: {.content-visible when-format="html"}
![סדרה המתכנסת ל-$L$, והאזור המוצלל הוא סביבת ה-$\varepsilon$ של $L$, כלומר הקטע $(L-\varepsilon,\,L+\varepsilon)$; כל האיברים שהאינדקס שלהם גדול או שווה ל-$N$ נמצאים בתוכה, ומחוצה לה נשארו רק האיברים המסומנים באדום](figures/c07_fig09.png){#fig-seq-eps-band width="80%" fig-align="center"}
:::
...
התרגילים הבאים לקוחים מדפי התרגילים של הקורס, ואפשר לפתור את כולם בעזרת ההגדרה הפורמלית בלבד. בגרסה המקוונת, לחיצה על ״אמת״ או על ״שקר״ פותחת מיד את הפתרון; בכל שאר התרגילים אפשר לפתוח את הפתרון בלחיצה.
...
**אמת או שקר.** בארבעת התרגילים הבאים $\{a_n\}_{n=1}^{\infty}$ ו-$\{b_n\}_{n=1}^{\infty}$ הן סדרות ממשיות כלשהן.
...
::: {.thmsol .foldable}
**שקר.** ניקח $a_n = \left(-1\right)^{n}$ ו-$b_n = -\left(-1\right)^{n}$. אף אחת מהן אינה מתכנסת, בדיוק לפי הטיעון שראינו למעלה עבור הסדרה המתחלפת. אבל לכל $n$ מתקיים $a_n + b_n = 0$, ולכן הסכום הוא הסדרה הקבועה $0$, שמתכנסת ל-$0$.
:::
...
::: {.thmsol .foldable}
**אמת.** נניח בשלילה שגם $\{a_n+b_n\}$ מתכנסת, ונסמן $a_n \to A$ וכן $a_n+b_n \to S$. נראה שאז $b_n \to S-A$, בסתירה להנחה ש-$\{b_n\}$ אינה מתכנסת.
...
::: {.thmsol .foldable}
**שקר.** אותה דוגמה נגדית שבתרגיל הראשון: עבור $a_n = \left(-1\right)^{n}$ ו-$b_n = -\left(-1\right)^{n}$ הסכום הוא הסדרה הקבועה $0$, שמתכנסת, ואילו אף אחת משתי הסדרות עצמן אינה מתכנסת.
:::
...
::: {.thmsol .foldable}
**אמת.** נוכיח את הטענה השקולה: אם שתי הסדרות מתכנסות, אז גם הסכום מתכנס.
